...
typedef volatile struct GC_Thread_Rep * GC_vthread;
  ...
  /* (when a thread is detached by DllMain()) and                       */
...
#         ifndef GC_NO_THREADS_DISCOVERY
            if (GC_win32_dll_threads) {
              ...
              /* about to) by DllMain().  And, DETACH_THREAD_LOCK()     */
...
#       ifndef GC_NO_THREADS_DISCOVERY
          if (GC_win32_dll_threads
            ...
            /* a race with DllMain() in setting the flags is fine.      */
            GC_has_pending_delete_threads = TRUE;
            t -> pending_delete_thread = TRUE;
            return;
          }
    ...
    if (GC_win32_dll_threads) {
      ...
      for (i = 0; i <= my_max; i++) {
        ...
        if (t -> stack_base != 0 && t -> thread_blocked_sp == NULL
            && t -> id != thread_id) {
          /* A race with DllMain() in accessing */
...
# ifndef GC_NO_THREADS_DISCOVERY
    ...
    /* A race with DllMain() in accessing pending_delete_thread is      */
    /* fine.                                                            */
